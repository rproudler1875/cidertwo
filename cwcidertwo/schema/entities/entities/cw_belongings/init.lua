	...
	if (!Clockwork.kernel:IsShuttingDown()) then
		...
		if (self.inventory) then
			for k, v in pairs(self.inventory) do
				if (v > 0) then
					for i = 1, v do
						local item = Clockwork.entity:CreateItem(nil, k, self:GetPos() + Vector(0, 0, math.random(1, 48)), self:GetAngles());
						
						if (IsValid(item)) then
							Clockwork.entity:CopyOwner(self, item);
						end;
					end;
				end;
			end;
		...
		if (self.cash and self.cash > 0) then
			Clockwork.entity:CreateCash(nil, self.cash, self:GetPos() + Vector(0, 0, math.random(1, 48)));
		end;
