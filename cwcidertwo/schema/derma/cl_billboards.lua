...
function PANEL:Rebuild()
	...
	for k, v in ipairs(CiderTwo.billboards) do
	...
	if (#availableBillboards > 0) then
		local label = vgui.Create("cwInfoText", self);
			label:SetText("Each billboard will cost you "..Clockwork.kernel:FormatCash(60)..".");
		...
		for k, v in ipairs(availableBillboards) do
			...
			if (v.data) then
				...
				function takeDownButton.DoClick(button)
					Clockwork.datastream:Start("TakeDownBillboard", v.id);
				end;
			else
			...
			function button.DoClick(button)
				Clockwork.datastream:Start("UpdateBillboard", {
		...
		local label = vgui.Create("cw_InfoText", self);
...
vgui.Register("cw_Billboards", PANEL, "DFrame");