...
local COMMAND = Clockwork.command:New("911");
...
function COMMAND:OnRun(player, arguments)
	...
	if (playerCallerID and playerCallerID != "") then
		...
		if (!player.nextSend911 or curTime >= player.nextSend911) then
			...
			Clockwork.player:Notify(player, "You can not call 911 for another "..math.Round(math.ceil(player.nextSend911 - curTime)).." second(s)!");
		end;
	else
